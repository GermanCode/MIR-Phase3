 ...
 * Tokenizer
 *
 * The Tokenizer class tokenizes a signle document or a set of documents which have 
 * names in format "doc[i]" where [i] is the document identification number.
 * 
...
public class Tokenizer {
	...
	private static final String swPath = "D:\\Documents\\Course\\MIR\\Project\\NPL\\stopwords";


//	private static String urlRegex = "((((https?|ftp|file)://)|(www\\.))[-a-zA-Z0-9+&@#/%?=~_|!:,.;]*[-a-zA-Z0-9+&@#/%=~_|])";
...
//	private static String emailRegex = "([_A-Za-z0-9-\\+]+(\\.[_A-Za-z0-9-]+)*@[A-Za-z0-9-]+(\\.[A-Za-z0-9]+)*(\\.[A-Za-z]{2,}))";
	...
	/**
	 * @param source Folder path in case of using Tokenizer on a number of docs
	 * and a file path otherwise.
	 * @param allDocs True is case of using Tokenizer on a number of docs and 
	 * false otherwise.
	 */
	...
	/**
	 * Returns next token after stemming and checking whether current token is 
	 * a stop word or not.
	 * <p>
	...
	public String nextToken() {
		...
		while (true) {
			...
			if (result == null) {
				if (!scanner.hasNext()) {
					if (this.tokenizationMode == TokenizationMode.FolderFiles && initializeNewFile()) {
						return nextToken();
					} else {
						scanner.close();
						tokenNumber ++;
		...
		if (result.equals(".") || stopWords.get(result.toLowerCase()) != null) {
			return nextToken();
		}
		tokenNumber ++;
	...
	private boolean initializeNewFile() {
		...
		docNumber = Integer.parseInt(filePath.substring(filePath.lastIndexOf("\\") + 1));
		try {
